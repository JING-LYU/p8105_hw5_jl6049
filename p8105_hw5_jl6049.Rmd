...
x = iris_with_missing$Sepal.Length
...
b = map2(.x = a, .y = a, ~fill_missing(.x, .y))


k= 
  iris_with_missing %>% 
  map_df(Sepal.Length,fill_missing)
  

mutate(
    mean_Length = round(mean(Sepal.Length, na.rm = TRUE),1),
    Sepal.Length = map2(.x = Sepal.Length, .y = mean_Length, ~fill_missing(.x, .y))) %>% 
  
  mutate(Species = map(Species, fill_missing )) %>% 
  
  

pull(iris_with_missing[])
